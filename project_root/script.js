...
socket.onmessage = function (event) {
    ...
    handleRemoteDrawing({ ...data, userId: data.userId });
};
...
function handleRemoteDrawing(data) {
    ...
    switch (type) {
        case 'draw':
            ...
            // Check if we need to start a new stroke for this user
            if (!currentRemoteStrokes[userId] || currentRemoteStrokes[userId].id !== strokeId) {
                currentRemoteStrokes[userId] = new Stroke();
        ...
        case 'drawEnd':
            ...
            delete currentRemoteStrokes[userId];
            break;
